BrynerCoin Core version *0.15.1* is now available from:

  <https://download.brynercoin.org/brynercoin-0.15.1/>
...
A number of changes to the way BrynerCoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 raps per kB (0.00001 BRY). This is to prepare for dropping the minimum transaction fee to 0.1 raps per kB (0.0001 BRY) in the next release.
