...
const char * const BITCOIN_CONF_FILENAME = "brynercoin.conf";
const char * const BITCOIN_PID_FILENAME = "brynercoind.pid";
...
#else
    const char* pszModule = "brynercoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BrynerCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BrynerCoin";
#else
    // Unix
    return pathRet / ".brynercoin";
#endif
#endif
}
